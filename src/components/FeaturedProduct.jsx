...
const FeaturedProduct = () => {
  ...
  const featuredProduct = async () => {
    try {
      const {message, success, data} = await axios.get('/home/featured-product');
      if (success) {
        setProducts(data);
      }
    } catch (error) {
  ...
  return (
    <div className='max-w-360 mx-auto lg:px-8 px-5 py-10'>
      <h2 className='text-green-400 text-4xl font-medium pb-10'>Featured <span className='text-gray-700'>Product</span> </h2>
